...
func (rq *ResearchQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Research, init func(*Research), assign func(*Research, *Event)) error {
	...
	for _, n := range neighbors {
		fk := n.research_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "research_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "research_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
