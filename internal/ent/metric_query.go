...
func (mq *MetricQuery) QueryEvents() *EventQuery {
...
func (mq *MetricQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (mq *MetricQuery) FirstIDX(ctx context.Context) string {
...
func (mq *MetricQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (mq *MetricQuery) OnlyIDX(ctx context.Context) string {
...
func (mq *MetricQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (mq *MetricQuery) IDsX(ctx context.Context) []string {
	ids, err := mq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (mq *MetricQuery) WithEvents(opts ...func(*EventQuery)) *MetricQuery {
	query := (&EventClient{config: mq.config}).Query()
...
func (mq *MetricQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Metric, error) {
	...
	if query := mq.withEvents; query != nil {
		if err := mq.loadEvents(ctx, query, nodes,
			func(n *Metric) { n.Edges.Events = []*Event{} },
			func(n *Metric, e *Event) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range mq.withNamedEvents {
		if err := mq.loadEvents(ctx, query, nodes,
			func(n *Metric) { n.appendNamedEvents(name) },
			func(n *Metric, e *Event) { n.appendNamedEvents(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (mq *MetricQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Metric, init func(*Metric), assign func(*Metric, *Event)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Metric)
	for i := range nodes {
	...
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(metric.EventsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.metric_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "metric_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "metric_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (mq *MetricQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(metric.Table, metric.Columns, sqlgraph.NewFieldSpec(metric.FieldID, field.TypeString))
	_spec.From = mq.sql
...
func (mq *MetricQuery) WithNamedEvents(name string, opts ...func(*EventQuery)) *MetricQuery {
	query := (&EventClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if mq.withNamedEvents == nil {
		mq.withNamedEvents = make(map[string]*EventQuery)
	}
	mq.withNamedEvents[name] = query
	return mq
}
